...
export class KafkaOb1Controller implements OnModuleInit {
  ...
  async handleSystemMessages(
    ...
    try {
      ...
      this.logger.debug(
        `Returning error response with error headers: ${JSON.stringify(errorHeaders)}`,
      );
      return {
        key: '',
    ...
    if (messageHeaders.destinationService !== SERVICE_NAME) {
      ...
      return {
        messageStatus: 'error',
        errorMessage: `Message not intended for this service (${SERVICE_NAME}) but instead for ${messageHeaders.destinationService}`,
      };
    }
    ...
    try {
      const result = await this.kafkaOb1ProcessingService.processRequest(
        message,
        context,
      );

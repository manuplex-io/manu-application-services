...
@Injectable()
export class googleSpaceService {
  private readonly logger = new Logger(googleSpaceService.name);
  ...
  async getSpaceInfo(functionInput: any, context: KafkaContext) {
    ...
    try {
      ...
      const refreshToken = process.env.SPACEREFRESHTOKEN
    ...
    //   }
      ...
      const CRUDFunctionInput = {
        ...
        CRUDBody: executeDto,
        routeParams: { promptId: process.env.GETSPACEINFO },
      }; //CRUDFunctionInput
      ...
      const { spaceMessages } = parsedMessage; // This should be array of messages including relevant details for each message

      if (spaceMessages && spaceMessages.length > 0) {
        ...
        // const allspaceMessages = {
        //   summaries: spaceMessages,
        // };
        ...
        await this.sendCsvAttachment(ticketId,spaceMessages);

        }

        this.logger.log('Space summary prepared and Jira ticket created');
      }
  ...
  async getGoogleAccessToken(refreshToken: string): Promise<string | null> {
    try {
      const response = await axios.post('https://oauth2.googleapis.com/token', null, {
        params: {
          client_id: process.env.SPACE_CLIENT_ID,
          client_secret: process.env.SPACE_CLIENT_SECRET,
      ...
      if (response.data.access_token) {
        ...
        this.logger.error('No access token returned from Google', response.data);
        return null;
      }
    } catch (error) {
      this.logger.error('Error fetching Google access token', error.response?.data || error.message);
      return null;
    }
  }
  ...
  async generateConciseSummary(
        ...
        const executeDto = {
          userPromptVariables: {
            messagesJson: JSON.stringify(spaceMessages),
            title,
            description,
          },
          // toolENVInputVariables,
        ...
        const CRUDFunctionInput = {
          ...
          CRUDRoute: CRUDPromptRoute.EXECUTE_WITHOUT_USER_PROMPT,
          CRUDBody: executeDto,
          routeParams: { promptId: process.env.GENERATESPACESUMMARY },
        };
        ...
        if (response.messageContent.content) {
          ...
          const { ConciseSummary } = response.messageContent.content;
          return {
  ...
  async sendCsvAttachment(
    ...
    try {
      ...
      const csvContent = csvData
        .map((row) => {
          const escapedSender = `"${row.Sender.replace(/"/g, '""')}"`;
          const escapedMessage = `"${row.Text.replace(/"/g, '""').replace(/\n/g, ' ')}"`;
          return `${escapedSender},${escapedMessage}`;
        })
        .join('\n');
  
      // Write to the CSV file
      fs.writeFileSync(tempFilePath, `"Sender","Message"\n${csvContent}`);
